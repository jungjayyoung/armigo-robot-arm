...
집게로 사용하는 Slave ID `2`에는 Torque Limit `300/1023`(약 29%)을 적용해
물체를 끝까지 압착할 때 모터가 장시간 스톨 상태에 머무르지 않도록 합니다.
나머지 축은 기존 Torque 설정을 사용합니다.
...
3. 유효 범위 `10~17cm`가 3회 연속 확인되면 `sharp_detected`를 켭니다.
4. 범위를 벗어난 샘플이 3회 연속 확인될 때 감지 상태를 해제합니다.

ADC는 40ms 주기로 측정하므로, 감지 해제에도 약 120ms의 노이즈 억제를
적용해 Leader의 5초 Auto 카운트다운이 순간적인 센서 흔들림으로 초기화되지
않도록 합니다.
